 ...
 */
export const listBillApprovalPolicies = {
    name: "list_bill_approval_policies",
    description: "List bill approval policies. Policies define rules for when bills require approval before payment. Requires Administrator, Accountant, or Approver role permissions.",
    inputSchema: {
        type: "object",
        properties: {
            limit: {
                type: "number",
                description: "Maximum number of policies to return per page (default: 50, max: 100)",
                default: 50,
            },
            page: {
                type: "string",
 ...
 */
export const createBillApprovalPolicy = {
    name: "create_bill_approval_policy",
    description: "Create a new bill approval policy. Policies define rules (e.g., amount thresholds) that trigger approval requirements.",
    inputSchema: {
        type: "object",
        properties: {
            policyName: {
            ...
            rules: {
                type: "array",
                description: "Array of rules that trigger this policy. Each rule has a key (field), op (operator), and values.",
                items: {
                    type: "object",
                    properties: {
                        key: {
                            type: "string",
                            description: "Field to evaluate (e.g., 'amount', 'vendorId')",
                        },
                        op: {
                            type: "string",
                            description: "Operator for comparison (e.g., 'gt', 'eq', 'in')",
                        },
                        values: {
                            type: "array",
            ...
            approvers: {
                type: "array",
                description: "Array of user IDs who can approve bills matching this policy (user IDs start with '006')",
                items: {
                    type: "string",
                },
            },
            minApprovers: {
                type: "number",
                description: "Minimum number of approvers required (default: 1)",
                default: 1,
            },
        },
    ...
    handler: async (args, client) => {
        try {
            ...
            const parsedApprovers = typeof approvers === "string" ? JSON.parse(approvers) : approvers;
            const body = {
                ...rest,
                rules: parsedRules,
 ...
 */
export const updateBillApprovalPolicy = {
    ...
    handler: async (args, client) => {
        try {
            ...
            const policy = await client.put(`/bill-approvals/policies/${policyId}`, body);
            return {
                success: true,
                data: policy,
            };
 ...
 */
export const deleteBillApprovalPolicy = {
    ...
    handler: async (args, client) => {
        try {
            const result = await client.delete(`/bill-approvals/policies/${args.policyId}`);
            return {
                success: true,
                data: result,
            };
 ...
 */
export const listPendingBillApprovals = {
    name: "list_pending_bill_approvals",
    description: "List bills that are pending approval for the current user. Returns only bills currently awaiting the authenticated user's approval — an empty result is normal when no bills need approval. Requires Administrator, Accountant, or Approver role permissions.",
    inputSchema: {
        type: "object",
        properties: {
            limit: {
                type: "number",
                description: "Maximum number of pending approvals to return per page (default: 50, max: 100)",
                default: 50,
            },
            page: {
                type: "string",
    ...
    handler: async (args, client) => {
        ...
        try {
            const params = buildListParams({ max: limit, page });
            const pending = await client.get("/bill-approvals/pending-user-approvals", params);
            return {
                success: true,
                data: pending,
            };
...
export const approveBill = {
    name: "approve_bill",
    description: "Approve a bill that is pending approval. Requires full API access (not available with sync token). The authenticated user must be an approver for the bill.",
    inputSchema: {
        type: "object",
        properties: {
            billId: {
 ...
 */
export const denyBill = {
    name: "deny_bill",
    description: "Deny/reject a bill that is pending approval. Requires full API access (not available with sync token). The authenticated user must be an approver for the bill.",
    inputSchema: {
        type: "object",
        properties: {
            billId: {
...
//# sourceMappingURL=bill-approvals.js.map