 ...
 * List vendor credits
 */
export const listVendorCredits = {
    name: "list_vendor_credits",
    description: "List vendor credits with filtering, sorting, and pagination. Vendor credits adjust the amount owed to vendors.",
    inputSchema: {
        type: "object",
        properties: {
            limit: {
                type: "number",
                description: "Maximum number of vendor credits to return per page (default: 50, max: 100)",
                default: 50,
            },
            page: {
                type: "string",
            ...
            sort: {
                type: "string",
                description: "Sort order in 'field:direction' format. Sortable fields: creditDate, amount, createdTime, updatedTime. Default: 'createdTime:desc'.",
            },
            vendorId: {
                type: "string",
            ...
            archived: {
                type: "boolean",
                description: "Filter by archived status. Use false for active credits, true for archived.",
            },
            createdAfter: {
                type: "string",
                description: "Filter vendor credits created on or after this date (ISO 8601 format, e.g. 2025-01-01)",
            },
            createdBefore: {
                type: "string",
                description: "Filter vendor credits created on or before this date (ISO 8601 format, e.g. 2025-12-31)",
            },
        },
        required: [],
    },
    handler: async (args, client) => {
        const { limit, page, sort, vendorId, archived, createdAfter, createdBefore } = args;
        try {
            const filters = [];
            if (vendorId)
 ...
 */
export const createVendorCredit = {
    name: "create_vendor_credit",
    description: "Create a new vendor credit to adjust the amount owed to a vendor",
    inputSchema: {
        type: "object",
        properties: {
            vendorId: {
            ...
            creditDate: {
                type: "string",
                description: "Date of the credit (YYYY-MM-DD format). Defaults to today if omitted.",
            },
            referenceNumber: {
                type: "string",
            ...
            vendorCreditLineItems: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        chartOfAccountId: {
                            type: "string",
                            description: "Chart of account ID for expense categorization (starts with '0ca'). Set per line item.",
                        },
                    },
                    required: ["amount"],
                },
    ...
    handler: async (args, client) => {
        try {
            ...
            const parsedLineItems = typeof vendorCreditLineItems === "string"
                ? JSON.parse(vendorCreditLineItems)
                : vendorCreditLineItems;
            // Wrap line-item chartOfAccountId into classifications object
            const transformedLineItems = parsedLineItems.map(({ chartOfAccountId, ...lineRest }) => {
                if (chartOfAccountId) {
 ...
 */
export const updateVendorCredit = {
 ...
 */
export const archiveVendorCredit = {
