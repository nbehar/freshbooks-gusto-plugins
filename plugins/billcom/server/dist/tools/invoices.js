...
export const listInvoices = {
    name: "list_invoices",
    description: "List invoices (accounts receivable) with filtering, sorting, and pagination. Returns newest first by default. Results contain customer IDs — use get_customer to resolve customer names.",
    inputSchema: {
        type: "object",
        properties: {
            limit: {
                type: "number",
                description: "Maximum number of invoices to return per page (default: 50, max: 100)",
                default: 50,
            },
            page: {
                type: "string",
            ...
            sort: {
                type: "string",
                description: "Sort order in 'field:direction' format. Sortable fields: invoiceNumber, invoiceDate, dueDate, totalAmount, createdTime, updatedTime, archived. Default: 'createdTime:desc'. Example: 'dueDate:asc'",
            },
            customerId: {
                type: "string",
            ...
            createdAfter: {
                type: "string",
                description: "Filter invoices created on or after this date (ISO 8601 format, e.g. 2025-01-01)",
            },
            createdBefore: {
                type: "string",
                description: "Filter invoices created on or before this date (ISO 8601 format, e.g. 2025-12-31)",
            },
        },
        required: [],
    },
    handler: async (args, client) => {
        const { limit, page, sort, customerId, archived, createdAfter, createdBefore } = args;
        try {
            const filters = [];
            if (customerId)
...
export const createInvoice = {
    ...
    handler: async (args, client) => {
        try {
            ...
            const parsedLineItems = typeof invoiceLineItems === "string" ? JSON.parse(invoiceLineItems) : invoiceLineItems;
...
export const sendInvoice = {
    name: "send_invoice",
    description: "Send an invoice to the customer via email. Requires full API access (not available with sync token).",
    inputSchema: {
        type: "object",
        properties: {
            id: {
            ...
            replyToUserId: {
                type: "string",
                description: "Optional Bill.com AP/AR user ID for the reply-to address (starts with '00U'). Omit to use the default.",
            },
        },
...
export const generateInvoicePaymentLink = {
    ...
    description: "Generate a payment link for an invoice that customers can use to pay online. The link allows customers to pay without needing a BILL account.",
    inputSchema: {
        type: "object",
        properties: {
            id: {
