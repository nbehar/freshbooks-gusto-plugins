...
export const listChartOfAccounts = {
    ...
    description: "List chart of accounts entries with filtering, sorting, and pagination. These are GL account categories for classifying financial transactions.",
    inputSchema: {
        type: "object",
        properties: {
            limit: {
                type: "number",
                description: "Maximum number of accounts to return per page (default: 50, max: 100)",
                default: 50,
            },
            page: {
                type: "string",
            ...
            sort: {
                type: "string",
                description: "Sort order in 'field:direction' format. Sortable fields: accountNumber, createdTime, updatedTime. Default: 'createdTime:desc'.",
            },
            name: {
                type: "string",
                description: "Filter accounts whose name starts with this value (uses starts-with matching)",
            },
            archived: {
                type: "boolean",
                description: "Filter by archived status. Use false for active accounts, true for archived.",
            },
        },
        required: [],
    },
    handler: async (args, client) => {
        ...
        try {
            ...
            const accounts = await client.get("/classifications/chart-of-accounts", params);
            return {
                success: true,
                data: accounts,
            };
 ...
 */
export const getChartOfAccount = {
    ...
    handler: async (args, client) => {
        try {
            const account = await client.get(`/classifications/chart-of-accounts/${args.id}`);
            return {
                success: true,
                data: account,
            };
...
export const createChartOfAccount = {
    ...
    handler: async (args, client) => {
        try {
            ...
            const account = await client.post("/classifications/chart-of-accounts", body);
            return {
                success: true,
                data: account,
            };
...
export const updateChartOfAccount = {
    ...
    handler: async (args, client) => {
        try {
            ...
            const account = await client.patch(`/classifications/chart-of-accounts/${id}`, updateData);
            return {
                success: true,
                data: account,
            };
 ...
 */
export const archiveChartOfAccount = {
    ...
    handler: async (args, client) => {
        try {
            const result = await client.post(`/classifications/chart-of-accounts/${args.id}/archive`);
            return {
                success: true,
                data: result,
            };
 ...
 */
export const restoreChartOfAccount = {
    ...
    handler: async (args, client) => {
        try {
            const result = await client.post(`/classifications/chart-of-accounts/${args.id}/restore`);
            return {
                success: true,
                data: result,
            };
