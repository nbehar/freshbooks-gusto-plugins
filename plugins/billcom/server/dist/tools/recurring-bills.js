 ...
 */
export const listRecurringBills = {
    name: "list_recurring_bills",
    description: "List recurring bill templates with filtering, sorting, and pagination.",
    inputSchema: {
        type: "object",
        properties: {
            limit: {
                type: "number",
                description: "Maximum number of recurring bills to return per page (default: 50, max: 100)",
                default: 50,
            },
            page: {
                type: "string",
            ...
            sort: {
                type: "string",
                description: "Sort order in 'field:direction' format. Only 'archived' is supported (e.g., 'archived:asc' or 'archived:desc'). Omit for default API ordering.",
            },
            vendorId: {
                type: "string",
...
export const createRecurringBill = {
    ...
    handler: async (args, client) => {
        try {
            const { schedulePeriod, scheduleFrequency, nextDueDate, endDate, daysInAdvance, recurringBillLineItems, autoPayment, bankAccountId, ...rest } = args;
            // Parse line items if they arrive as a JSON string
            const parsedLineItems = typeof recurringBillLineItems === "string"
...
export const updateRecurringBill = {
    ...
    handler: async (args, client) => {
        try {
            const { id, schedulePeriod, scheduleFrequency, nextDueDate, endDate, daysInAdvance, recurringBillLineItems, ...rest } = args;
            const body = { ...rest };
            ...
            if (recurringBillLineItems) {
                const parsedLineItems = typeof recurringBillLineItems === "string"
                    ? JSON.parse(recurringBillLineItems)
                    : recurringBillLineItems;
                body.recurringBillLineItems = parsedLineItems.map(({ chartOfAccountId, ...lineRest }) => {
                    if (chartOfAccountId) {
                        return { ...lineRest, classifications: { chartOfAccountId } };
                    }
                    return lineRest;
 ...
 */
export const archiveRecurringBill = {
 ...
 */
export const restoreRecurringBill = {
