...
function getBillConfig() {
    ...
    if (authType !== "sync_token" && authType !== "full_access" && authType !== "session_token") {
        throw new Error(`Invalid BILL_AUTH_TYPE: "${authType}". Must be "sync_token", "full_access", or "session_token".`);
    }
    ...
    if (authType === "session_token") {
        ...
        if (!sessionToken) {
            throw new Error("BILL_SESSION_TOKEN environment variable is required for session_token auth type.");
        }
        return {
            devKey,
    ...
    if (!username || !password || !organizationId) {
        throw new Error("Bill.com credentials not found. Please set BILL_USERNAME, BILL_PASSWORD, and BILL_ORGANIZATION_ID environment variables.");
    }
    return {
        devKey,
        username,
